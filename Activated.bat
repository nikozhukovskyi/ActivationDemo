...
for /f "tokens=2 delims==" %%i in ('wmic path SoftwareLicensingService get OA3xOriginalProductKey /value ^| find "OA3xOriginalProductKey"') do (
    set "licenseType=OEM"
)

if "%currentStatus%"=="1" (
    if not "%licenseType%"=="" (
        echo [STATUS] Windows is already activated via OEM key.
    ) else (
        echo [STATUS] Windows is already activated (Retail or KMS).
    )
